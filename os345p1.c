...
int P1_shellTask(int argc, char* argv[])
{
	...
	while (1)
	{
		...
		// ?? must use malloc for argv storage!
		{
			...
			// init arguments
			newArgc = 1;
			myArgv[0] = token = inBuffer;				// point to input string
			for (i=1; i<MAX_ARGS; i++) myArgv[i] = 0;

			// parse input string
			enum states { DULL, IN_WORD, IN_STRING } state = DULL;

			for (token = inBuffer; *token != '\0'; token++) {
					if(state != IN_STRING)
					{
						*token = tolower(*token);
					}
					c = (unsigned char) *token; /* convert to unsigned char for is* functions */
			    switch (state) {
			    case DULL: /* not in a word, not in a double quoted string */
			        ...
			        if (c == '"') {
									*token = 0;
			            state = IN_STRING;
			            myArgv[current_arg] = token + 1; /* word starts at *next* char, not this one */
									current_arg++;
			            continue;
			        }
			        state = IN_WORD;
			        myArgv[current_arg] = token; /* word starts here */
							current_arg++;
			        continue;
			...
			char *lastArg = myArgv[current_arg - 1];
			if (!strcmp(lastArg, "&"))
			{
				background = TRUE;
				current_arg--;
			}
			else if (!strcmp(lastArg + strlen(lastArg) - strlen("&"), "&"))
			{
				background = TRUE;
				*(lastArg + strlen(lastArg) - strlen("&")) = 0;
			}
			newArgc = current_arg;
			newArgv = malloc(sizeof(char*) * (newArgc + 1));
			for(int i = 0; i < newArgc; i++)
			{
				newArgv[i] = malloc((strlen(myArgv[i]) + 1) * sizeof(char));
				strcpy(newArgv[i], myArgv[i]);
			}
			/*while(token = strchr(cmdline, delim))
			{
				arglen = strlen(token);
				malloc(cmdline[current_arg], arglen * sizeof(char));
				strcpy(cmdline[current_arg], token);
				while(*token == ' ')
				{
					token++;
				}
		...
		for (found = i = 0; i < NUM_COMMANDS; i++)
		{
				 ...
				 !strcmp(newArgv[0], commands[i]->shortcut))
			{
				...
				// if there was an ambersand, we need to run in the background.
				int retValue = 0;
				if(background)
				{

					child_id = createTask(newArgv[0], (*commands[i]->func), MED_PRIORITY, newArgc, newArgv);
					background = FALSE;
				}
				else
				{
...
int P1_add(int argc, char* argv[])
{
	...
	for(int i = 1; i < argc; i++){
		...
		else if(number[0] == 37){ //ascii value for %
			//BINARY
			number++;
			base = 2;
		}
		else if(number[strlen(number) - 1] == 'h')
		{
			//HEX
		...
		result += (int) strtol(number, NULL, base);
...
int P2_print_queue(int argc, char* argv[])
{
	print(&rq);
	return 0;
}
...
Command** P1_init()
{
	...
	commands[i++] = newCommand("add", "ad", P1_add, "Add numbers");
	commands[i++] = newCommand("args", "args", P1_args, "Lists arguments");
	...
	commands[i++] = newCommand("printQueue", "pq", P2_print_queue, "Print Ready Queue");
	...
	commands[i++] = newCommand("8frames", "8f", P4_8FramesTest, "8 Frames Test");
