...
extern struct pqueue rq;			// ready queue
					...
					char* argv[])				// task argument pointers
{
	...
	for (tid = 0; tid < MAX_TASKS; tid++)
	{
		if (tcb[tid].name == 0)
		{
			...
			if(tid == 0) tcb[tid].parent = -1;
			tcb[tid].argc = argc;			// argument count

			// ?? malloc new argv parameters

			/***************MALLOC'ING ARGV************************/
			tcb[tid].argv = malloc((tcb[tid].argc + 1) * sizeof(char*));
			for(int i = 0; i < tcb[tid].argc; i++){
				tcb[tid].argv[i] = malloc((strlen(argv[i]) + 1) * sizeof(char));
				strcpy(tcb[tid].argv[i], argv[i]);
				*(tcb[tid].argv[i] + strlen(argv[i])) = 0;
			}
			/*****************************************************/
