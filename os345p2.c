...
int P2_project2(int argc, char* argv[])
{
	...
	createTask("I'm Alive",				// task name
					...
					2,							// task argc
					aliveArgv);				// task argument pointers
					//add 9 more tasks with high priority

	for(int i = 0; i < 9; i++)
	{
		createTask("Ten Second Ticks", tenSecTask, HIGH_PRIORITY, 2, aliveArgv);
	}
	return 0;
} // end P2_project2
...
// ?? 3) If blocked, indicate which semaphore
	struct pqueue temp;
	temp.data = (pqElement*)malloc(MAX_TASKS * sizeof(pqElement));
	initialize(&temp);
	...
	for (i=0; i<MAX_TASKS; i++)
	{
		if (tcb[i].name)
		{
			...
			task.tid = i;
			task.priority = tcb[i].priority;
			enqueue(&temp, task);
		}
	}

	while((i = (dequeue(&temp, -1)).tid) != -1)
	{
